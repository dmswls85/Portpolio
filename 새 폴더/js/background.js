...
float map(vec3 p) {
  // Center sphere
  float d = sdSphere(p - mouseDir * 5., 0.5);

  // Rotate world
  p.xz *= rot(sin(time * 0.3) * 0.4);  
  p.xy *= rot(time * 0.3);  
  
  // Place spheres
  float l = 4.4;
  d = min(d, sdSphere(p + vec3(0, 1, 1) * l, 1.0));
  d = min(d, sdSphere(p + vec3(0, -1, 1) * l * 2., 1.7));  
  d = min(d, sdSphere(p + vec3(0, 1, -1) * l, 1.2));  
  d = min(d, sdSphere(p + vec3(0, -1, -1) * l, 2.9));    
...
const vfx = new VFX({ pixelRatio: 0.8});
...
//  ctx.fillStyle = "white";
  ...
  for (const line of lines) {
    ctx.fillText(line, padding + rect.width * -1, padding + halfLeading + yOffset);    
